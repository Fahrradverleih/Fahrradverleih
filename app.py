    ...
    for rad in raeder:
        html += f"""
            <div class="card">
                ...
                <br><br>
                {'''<form action="/reservieren/''' + str(rad.id) + '''" method="POST" onsubmit="return validateForm(this)">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" id="dsgvo_''' + str(rad.id) + '''" name="dsgvo" required>
                        <label for="dsgvo_''' + str(rad.id) + '''">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" id="haftung_''' + str(rad.id) + '''" name="haftung" required>
                        <label for="haftung_''' + str(rad.id) + '''">
                ...
                </form>''' if rad.status == 'Verfügbar' else '<p style="color: #94a3b8; font-style: italic;">🔒 Aktuell nicht verfügbar</p>'}
            </div>
        """
    html += """
        </div>
...
def mitarbeiter():
    ...
    <body>
        ...
        <div class="tab" onclick="showTab('kunden')">👤 Kunden (""" + str(len(kunden)) + """)</div>
        <div class="tab" onclick="showTab('wartungen')">🔧 Wartungen (""" + str(len(wartungen)) + """)</div>
    ...
    for rad in raeder:
        html += f"""
                <tr>
                    ...
                    <td>{rad.marke}</td>
                    <td>{rad.modell}</td>
                    <td><span class="badge {rad.status.lower()}">{rad.status}</span></td>
                    <td>{rad.standort}</td>
                    <td>
    ...
    for w in wartungen:
        html += f"""
                <tr>
                    <td>{w.fahrrad_id}</td>
                    <td>{w.mitarbeiter}</td>
                    <td>{w.problem[:50]}{'...' if w.problem|length > 50 else ''}</td>
